...
from dynamic_obstacles import position_node
...
class Navigation:
    ...
    def align_ttbot(self, target=0):
        cmd_vel = Twist()
        cmd_vel.linear.x = 0.6 # setting linear velocity 0
    ...
    def move_ttbot(self, target_y=0, target_x=0): # starting goal pose = 0,0
        ...
        while error>0.08:
            d = ( (target_y-self.ttbot_pose.pose.position.y)**2 + (target_x-self.ttbot_pose.pose.position.x)**2)
            error = abs(d)
            PID_controller_obj = PidController(0.03, 0.006, 0.007, dt, 0.0, 0.4)
            ...
            if self.front < 1 && self.left < 1 && self.right <1:
