...
class AStar():
    ...
    def reconstruct_path(self,sn,en):
        starting_key = sn.name
        ending_key = en.name
        dist = self.dist[ending_key]
        u = ending_key
        path = [u]
        while u != starting_key:
            u = self.via[u]
            path.append(u)
        path.reverse()
        return path,dist
...
#'''
def a_star_planning(start_pt, goal_pt='0,0'):
    ...
    as_maze = AStar(mp.map_graph) # AStar map object

    as_maze.solve_map(mp.map_graph.g[mp.map_graph.root],mp.map_graph.g[mp.map_graph.end]) # solvig the map
    ...
    print("Waypoints that have been identified for path planning = ", path_as)
